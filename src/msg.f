! msg.F -*-f90-*-
! Time-stamp: <2005-08-11 00:18:08 t-nissie>
...
subroutine msg(fd, filename, line, begin_or_end, message)
  ...
  integer, save      :: level(100)=0
  character(20)      :: fmt
  !---------------------------------------------------
  if (begin_or_end.eq.'') then
     write(fmt,'(a,i3.3,a)') '(', indent*level(fd), 'x,a)'
     write(fd,fmt) message
  else
     if (begin_or_end.eq.'END') then
        level(fd) = level(fd) - 1
     end if
     if (level(fd).gt.0) then
        write(fmt,'(a,i3.3,a)') '(', indent*level(fd), 'x,2a,i3,3a)'
     else
        fmt =                                           '(2a,i3,3a)'
     end if
     write(fd,fmt) filename, ':', line, ': ', begin_or_end, message
