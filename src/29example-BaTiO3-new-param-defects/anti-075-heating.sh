...
for temperature in `seq $temperature_start $temperature_step $temperature_goal`; do
    ...
    i=`expr $i + 1`
    filename=anti-075-heating`printf '%.3d' $i`-"$temperature"K
	...
	external_E_field = 0.00 0.00 0.00075
...
EOF
    ...
    cat $filename.avg >> anti-075-heating.avg
done

rm anti-075-heating???-[2-8]??K.*
