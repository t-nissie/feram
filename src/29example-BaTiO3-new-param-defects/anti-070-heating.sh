...
for temperature in `seq $temperature_start $temperature_step $temperature_goal`; do
    ...
    i=`expr $i + 1`
    filename=anti-070-heating`printf '%.3d' $i`-"$temperature"K
	...
	external_E_field = 0.00 0.00 0.00070
...
EOF
    ...
    cat $filename.avg >> anti-070-heating.avg
done

rm anti-070-heating???-[2-8]??K.*
