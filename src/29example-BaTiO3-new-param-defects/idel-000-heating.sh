#!/bin/sh
# idel-000-heating.sh
# Time-stamp: <2014-05-07 18:36:51 t-nissie>
# Author: Takeshi NISHIMATSU
##
rm -f idel-000-heating.avg
...
for temperature in `seq $temperature_start $temperature_step $temperature_goal`; do
    ...
    i=`expr $i + 1`
    filename=idel-000-heating`printf '%.3d' $i`-"$temperature"K
...
EOF
    ...
    cat $filename.avg >> idel-000-heating.avg
done

rm idel-000-heating???-???K.*
