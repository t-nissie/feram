...
#fit f_FX10(x) 'epit256x256x256.FX10.r1243.dat' via t1_FX10,p_FX10

f_SR16k(x) = t1_SR16k * ((1-p_SR16k)+p_SR16k/x)
t1_SR16k = 1800
p_SR16k  = 0.96
fit f_SR16k(x) 'epit256x256x256.SR16000.r1253.dat' using 1:2 via t1_SR16k,p_SR16k
...
plot      x t 'ideal' w l lt 5 lw 3,\
          'epit256x256x256.SR16000.r1253.dat' using ($1):(t1_SR16k/$2)  t ''        w p  lt 3 pt 4,\
          0.9<x && x<32.1 ? 1.0 / ((1-p_SR16k)+p_SR16k/x) : 1/0 t tp_SR16k  w l  lt 3 lw 3,\
          'epit256x256x256.E5-2670.r1258.dat' using ($1):(t1_E5_2670/$2)    t ''        w p  lt 2 pt 2,\
