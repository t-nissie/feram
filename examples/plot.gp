#!/usr/bin/env gnuplot
# Time-stamp: <2007-05-03 11:04:50 t-nissie>
...
set rmargin 18
plot '< grep  "s_Nose"         feram.out'\
     using 0:2 title '{/Times-Italic s}_{Nose}'           w l lt 4,\
     '< grep "pi_Nose"         feram.out'\
...
plot '< grep " total_energy  " feram.out' using 0:2 \
     title '{/Times-Italic E}_{total}'          w l

set terminal postscript landscape enhanced solid color 'Times-Roman' 16
set output 'plot.dipoR004.eps'
set autoscale y
set key max_step+(max_step-min_step)*0.13, 0.150 Left reverse spacing 1.3
plot '< grep "  dipoR" feram.out' using 0:2 title '{/Times-Italic u_x}(0,0,0)' w l,\
     '< grep "  dipoR" feram.out' using 0:3 title '{/Times-Italic u_y}(0,0,0)' w l,\
     '< grep "  dipoR" feram.out' using 0:4 title '{/Times-Italic u_z}(0,0,0)' w l

set terminal postscript landscape enhanced solid color 'Times-Roman' 16
set output 'plot.strain.eps'
#set yrange [-0.015:0.035]
set autoscale y
set key max_step+(max_step-min_step)*0.13, 0.02 Left reverse spacing 1.3
plot \
 '< grep " strain " feram.out' using 0:2 title '{/Times-Italic e_{xx}}' w l,\
 '< grep " strain " feram.out' using 0:3 title '{/Times-Italic e_{yy}}' w l,\
 '< grep " strain " feram.out' using 0:4 title '{/Times-Italic e_{zz}}' w l,\
 '< grep " strain " feram.out' using 0:5 title '{/Times-Italic e_{yz}}' w l,\
 '< grep " strain " feram.out' using 0:6 title '{/Times-Italic e_{zx}}' w l,\
 '< grep " strain " feram.out' using 0:7 title '{/Times-Italic e_{xy}}' w l lt 7

set terminal postscript landscape enhanced solid color 'Times-Roman' 16
set output 'plot.strain1.eps'
#set yrange [-0.015:0.035]
set autoscale y
set key max_step+(max_step-min_step)*0.13, 0.02 Left reverse spacing 1.3
#set ytics 0.001
plot '< grep " strain " feram.out' using 0:2 title '{/Times-Italic e_{xx}}' w l,\
     '< grep " strain " feram.out' using 0:3 title '{/Times-Italic e_{yy}}' w l,\
     '< grep " strain " feram.out' using 0:4 title '{/Times-Italic e_{zz}}' w l

set terminal postscript landscape enhanced solid color 'Times-Roman' 16
set output 'plot.strain4.eps'
set autoscale y
set key max_step+(max_step-min_step)*0.13, 0.02 Left reverse spacing 1.3
set ytics auto
plot \
 '< grep " strain " feram.out' using 0:5 title '{/Times-Italic e_{yz}}' w l,\
 '< grep " strain " feram.out' using 0:6 title '{/Times-Italic e_{zx}}' w l,\
 '< grep " strain " feram.out' using 0:7 title '{/Times-Italic e_{xy}}' w l
#Local variables:
